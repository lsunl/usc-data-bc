...
select staff.first_name, SUM(payment.amount) as total_amount from staff
JOIN payment
...
SELECT customer.first_name, customer.last_name, SUM(payment.customer_id) as total_paid
FROM customer
JOIN payment
...
SELECT * FROM actor
WHERE actor.actor_id IN (SELECT fa.actor_id FROM film_actor fa
	WHERE fa.film_id = 17);

-- 7c. You want to run an email marketing campaign in Canada, for which you will need the names and email addresses of all Canadian customers. Use joins to retrieve this information.
SELECT * from country; -- country_id =20

SELECT c.email FROM customer c
JOIN address a
ON a.address_id = c.address_id
WHERE a.city_id in (SELECT country_id FROM city WHERE country_id = 20)
GROUP BY c.email;
